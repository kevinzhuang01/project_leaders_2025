...
abstract: LANL's Venado, the first NVIDIA Grace-Hopper supercomputer in the US, will
  ...
  an exotic liquid cooling system for maximized system performance and reliability.
  To run large-scale simulations using GPUs on Venado, conventional HPC programming
  solutions include the mix of CUDA (GPU programming) and MPI (node communication),
  and the Kokkos ecosystem. However, these solutions usually require significant programming
  (e.g., C++ and MPI) and hardware expertise (e.g., GPU architecture). To strengthen
  LANL's HPC productivity on the rapidly evolving heterogeneous supercomputers with
...
Add additional details about Li_Tang here using markdown.
