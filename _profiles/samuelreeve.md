...
abstract: Particles (just points in a box) can be used to simulate a huge range of
  ...
  of those codes, and making sure the physics is correct (even with very different
  physics), we created a library to manage particles called Cabana. Cabana is made
  for high-performance computing (HPC) particle codes on all the hardware architectures
  (GPUs, etc.) across Department of Energy (using the Kokkos and MPI libraries). This
  project would focus on comparing the performance of recent Cabana applications on
...
Add additional details about Samuel_Reeve here using markdown.
